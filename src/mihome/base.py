...
XIAOMI_PASSWORD = 'ogywcqvc9nyqtuys'
...
class XiaomiConnection(object):
    ...
    def send(self, data, ip=None, port=None):
        ...
        print 'Sent to %s:%s %s' % (ip, str(port), data)
        return self.socket.sendto(
            data.encode("utf-8"), (ip, port)
        )

    def receive(self, **kwargs):
        print 'call receive '+str(kwargs)
        while True:
            # print 'wait for socket.recvfrom'
    ...
    def whois(self):
        self.send({'cmd': 'rwhois'}, port=self.SERVER_PORT)
